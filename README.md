...
**Goal**: to learn a policy that selects the optimal action for any given state.
...
**Model-Free** — No need to know the transition or reward model of the environment; the agent learns purely from experience.

**Off-policy learning** — The agent can update its policy using trajectories generated by a different behaviour policy. For example: if I observe an explorer stumble onto a million dollars, I can adjust my own (greedy) policy toward that discovery without ever having been the one exploring.

> **Note:** In realistic settings where time must be modelled, the state space can explode — approaches like state-representation learning become necessary. In this experiment that would be overkill; the goal is to keep the environment simple so the focus stays on the Active Learning strategy itself.

### 2. Five actions: four cardinal directions (↑, ↓, →, ←) plus ↗ up-right
...
### 3. Set ↗ up-right reward to 0 instead of a positive value
...
### 4. Use 50×50 grids in addition to 10×10
